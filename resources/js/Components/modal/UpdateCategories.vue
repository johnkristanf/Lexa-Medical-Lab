<script setup>
    ...
    const form = useForm({
        name: props.updateCategory?.name || '',
        description: props.updateCategory?.description || '',
    })

    // FORM SUBMISSION
    function submitForm() {
        if (!props.updateCategory?.id) {
            console.error('updateCategory is missing id')
            return
        }

        form.put(route('update.category', props.updateCategory.id), {
            preserveScroll: true,
            onSuccess: () => {
                toast.add({
                    severity: 'success',
